...
export function PostForm({
  ...
  function initialPoint(label: string | undefined, lat: number | null | undefined, lng: number | null | undefined): Point {
    return {
      label: label ?? "",
  ...
  const [origin, setOrigin] = useState<Point>(() =>
    initialPoint(initialValues?.origin, initialValues?.originLat, initialValues?.originLng)
  );
  const [destination, setDestination] = useState<Point>(() =>
    initialPoint(initialValues?.destination, initialValues?.destLat, initialValues?.destLng)
  );
  ...
  const [priceTouched, setPriceTouched] = useState(initialValues?.askingPriceCents != null);
  ...
  return (
    <form action={formAction} className="space-y-6 rounded-2xl border border-border bg-card p-6 shadow-sm">
      <LocationPicker
        ...
        useCurrentLocationAsDefault={!initialValues}
      />
      <LocationPicker
